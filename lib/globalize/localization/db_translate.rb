            ...
            class_eval <<-HERE
              ...
              def #{facet}_is_base?
                self['#{facet}_not_base'].nil?
							...
							private
							def globalize_dirty! fct
								@globalize_dirty ||= Hash.new
								@globalize_dirty[fct] = true
							end

							def globalize_dirty? fct
								@globalize_dirty &&	@globalize_dirty[fct]
							end
							
							def globalize_dirty_clear
								@globalize_dirty = nil
							end
      ...
      private
        ...
        def update_translation
          ...
          self.class.globalize_facets.each do |facet|
            next unless has_attribute?(facet) && globalize_dirty?(facet)
