    ...
    module TranslateObjectMethods # :nodoc: all
      ...
      private  
        ...
        def update_translation
          ...
          base_language_id = Locale.base_language.id
...
=end
      def find(*args)
        ...
        if options.has_key?(:include_translated) && !Locale.active
          raise ArgumentError, ":include_translated not valid option when no locale is set" 
        end
...
=end
        ...
        facets.each do |facet| 
          ...
          joins_clause  << " LEFT OUTER JOIN multilingual_translations AS #{facet_table_alias} " +
