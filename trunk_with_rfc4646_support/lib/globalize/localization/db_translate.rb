module Globalize # :nodoc:
  ...
  module DbTranslate  # :nodoc:
    ...
    mattr_accessor :keep_translations_in_model, :storage_method, :enable_fallbacks
...
=end
      def translates(*facets)
        ...
        case storage_method
          ...
          when :via_association
            translate_via_association(facets, options)
          else
...
=end
      ...
      protected
        ...
        def translate_via_association(facets, options)
          class_eval <<-HERE
            @@facet_options = {}
            @@translation_reflections = []
            @@translation_dyn_reflections = []
            @@aliases = {}
            ...
            class << self
              alias_method :untranslated_find, :find
              def find(*args)
                options = args.last.is_a?(Hash) ? args.pop : {}
                if (options.has_key?(:untranslated) && options[:untranslated] == true)
                  args << options
                  untranslated_find(*args)
                else
                  unless Locale.base?
                    @@facets.each do |f|
                      # Replaces table.untranslated_field or just untranslated_field with globalize_translations.text
                      r = Regexp.new("(\\\\A|\\\\s)(#\{table_name\}.#\{f\}|#\{f\})(\\\\z|\\\\s)")
                      options[:order].sub!(r, "\\\\1" + @@aliases[f] + "\\\\3") if options[:order]
                      options[:group].sub!(r, "\\\\1" + @@aliases[f] + "\\\\3") if options[:group]
                      options[:conditions].sub!(r, "\\\\1" + @@aliases[f] + "\\\\3") if options[:conditions].is_a?(String)
                    end

                    #If order is specified, only return results for current locale
                    if options[:order]
                      options[:conditions] ||= ''
                      case options[:conditions]
                        when String
                          options[:conditions] << " AND" unless options[:conditions].empty?
                          options[:conditions] << " globalize_translations.language_id = #\{Locale.language.id}"
                        when Array
                          options[:conditions].first << " AND" unless options[:conditions].first.empty?
                          options[:conditions].first << " globalize_translations.language_id = #\{Locale.language.id}"
                        else
                          #Unsupported
                          nil
                      end
                  ...
                  options[:include] = ([:translations] << options[:include]).compact.flatten
                  args << options
                  untranslated_find(*args)
                end
              end
            ...
            extend  Globalize::DbTranslate::ViaAssociationStorageClassMethods
          ...
          facets.each do |facet|
            ...
            fallback = (options[facet] && options[facet].key?(:fallback)) ?
                         ...
                         options[:fallback]
            fallback = Globalize::DbTranslate::enable_fallbacks ? fallback : false
            ...
            class_eval <<-HERE
              ...
              @@translation_reflections << :#{facet}_translations
              @@translation_dyn_reflections << :#{facet}_dyn_translations

              case @@dyn_table_aliases.size
                when 0
                  @@dyn_table_aliases[:#{facet}] = "globalize_translations"
                else
                  @@dyn_table_aliases[:#{facet}] = "#{facet}_dyn_translations_" + self.table_name
              end

              case @@aliases.size
                when 0
                  @@aliases[:#{facet}] = "globalize_translations.text"
                when 1
                  @@aliases[:#{facet}] = self.to_s.tableize + "_globalize_translations.text"
                else
                  @@aliases[:#{facet}] = self.to_s.tableize + "_globalize_translations" + "2" + ".text" # this is a guess
              end
              ...
              def #{facet}
                unless Locale.base?
                  translation = self.#{facet}_translations.detect{|tr| tr.language_id == Locale.language.id }
                    ...
                    if @@facet_options[:#{facet}][:fallback] && Globalize::DbTranslate::enable_fallbacks
                      Locale.active.fallbacks(true, true).each do |fallback|
                        unless Locale.base_language.code == fallback.code
                          ...
                          translation = read_attribute(:#{facet})
                          break if translation
                        end
                      end
                    end
                  ...
                  unless translation
                    if @@facet_options[:#{facet}][:base_as_default]
                      translation = read_attribute(:#{facet})
                    end
                  end
                  result = translation.nil? ? nil : (translation.kind_of?(::Globalize::ModelTranslation) ? translation.text : translation)
                else
                ...
                result.nil? ? nil : add_bidi(result, :#{facet})
              end
              alias :#{facet}_before_type_cast :#{facet}

              def #{facet}=(value)
                raise 'Globalize::Locale not active.' unless Locale.active?
                unless Locale.base?
                  ...
                  if value
                    ...
                    translation.destroy && self.#{facet}_translations(true) if translation
                  end
                else
            ...
            class_eval <<-HERE
              ...
              def #{facet}
                ...
                if !Locale.base? && translation.nil?
                  ...
                  if result.nil? && translation.nil? && @@facet_options[:#{facet}][:fallback] && Globalize::DbTranslate::enable_fallbacks
...
=end
    ...
    module ViaAssociationStorageClassMethods
      ...
      def method_missing(method_id, *arguments)
        if match = /find_(all_by|by)_([_a-zA-Z]\w*)/.match(method_id.to_s)
          ...
          unless Locale.base?
            conditions = []
            keys = attributes.keys.collect do |k|
               replaced = self.send(:class_variable_get, :@@dyn_table_aliases)[k.intern]
               replaced ? "#{replaced}.text" : k
            end
            conditions << keys.collect {|k| "#{k} = ?"}.join(' AND ')
            conditions += attributes.values

            attributes = conditions
          end

          extra_options = arguments[facets.size]
          extra_options ||= {}
          includes = self.send(:class_variable_get, :@@translation_dyn_reflections).dup
          extra_options[:include] = (includes << extra_options[:include]).compact.flatten
          ...
          includes = self.send(:class_variable_get, :@@translation_dyn_reflections).dup
          options = { :conditions => attributes, :include => includes}
          set_readonly_option!(options)
    ...
    end #ViaAssociationStorageClassMethods
