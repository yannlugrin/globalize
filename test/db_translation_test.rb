...
#  end
  ...
  def test_returned_base
	...
	# rows for fields that have not been translated
	def test_save_partial_record
    ...
    tr = Globalize::ModelTranslation.find(:first, :conditions =>
      [ "table_name = ? AND item_id = ? AND facet = ? AND language_id = ?",
      'globalize_products', prod.id, 'specs', Globalize::Locale.language.id ])
		assert_nil tr		# should be no translation
    tr = Globalize::ModelTranslation.find(:first, :conditions =>
      [ "table_name = ? AND item_id = ? AND facet = ? AND language_id = ?",
      'globalize_products', prod.id, 'description', Globalize::Locale.language.id ])
		assert_nil tr		# should be no translation
	  
		Globalize::Locale.set('en')
		prod.reload
		prod.specs = 'English specs'
		prod.description = 'English description'
    prod.save!
    ...
    tr = Globalize::ModelTranslation.find(:first, :conditions =>
      [ "table_name = ? AND item_id = ? AND facet = ? AND language_id = ?",
      'globalize_products', prod.id, 'description', Globalize::Locale.language.id ])
		assert_not_nil tr		# should be a translation
    tr = Globalize::ModelTranslation.find(:first, :conditions =>
      [ "table_name = ? AND item_id = ? AND facet = ? AND language_id = ?",
      'globalize_products', prod.id, 'specs', Globalize::Locale.language.id ])
		assert_nil tr		# still should be no translation    
	end
